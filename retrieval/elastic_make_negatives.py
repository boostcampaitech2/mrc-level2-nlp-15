...
class ElasticDPR:
    ...
    def get_doc_scores(self, query, k):
        ...
        for sample_id, idx_list in sample_id2idx.items():
            ...
            for idx in idx_list:
                ...
                prob = softmax(outputs.logits.to("cpu"), dim=1).squeeze()[
                    1
                ]  # [[0.2,0.8]]->[0.2,0.8]->0.8
    ...
    def generate_dataframe(self, dataset, es_k, dpr_k):
        ...
        if not is_test:
            print(
                f"ElasticSearch:{es_k}개 -> DPR:{dpr_k}개 추출 후 정답문서를 포함할 확률 {100*(included/total)}"
            )
...
class Elastic:
    def __init__(
        ...
        context_path="../data/wikipedia_documents_deduplicated2.json",
    ):
    ...
    def generate_dataset_for_dpr(self, dataset, k):
        ...
        for i in tqdm(range(len(dataset))):
            ...
            positive_included = (
                False  # Flag to check if 'positive context' is included in docs
            )
            candidate_doc_ids = []
            for doc in docs:
                item = dict()
