...
from transformers import BertModel,AutoConfig, BertPreTrainedModel, AdamW,AutoTokenizer,set_seed, TrainingArguments, get_linear_schedule_with_warmup,AutoModel
...
class gold_train:
    def __init__(self,args):
        ...
        self.train_dataset = {}
        self.train_dataset['context'] = [proprecessing(string) for string in train_dataset['context']]
        self.train_dataset['question'] = train_dataset['question']
        self.valid_dataset = {}
        self.valid_dataset['context'] = [proprecessing(string) for string in valid_dataset['context']]
    ...
    def train(self,epochs,learning_rate,train_dataset= None):
        ...
        q_seqs = self.tokenizer(train_dataset['question'],max_length=128, padding="max_length", truncation=True, return_tensors='pt')
        # print(q_seqs[0])
        ...
        q_seqs = self.tokenizer(self.valid_dataset['question'], padding="max_length", truncation=True, return_tensors='pt')
        p_seqs = self.tokenizer(self.valid_dataset['context'], padding="max_length", truncation=True, return_tensors='pt')
        # print(q_seqs[0])
        ...
        for epoch in range(epochs):
            ...
            for idx,data in enumerate(tqdm(train_loader)):
                step += 1
                p_inputs = {'input_ids': data[0].to(self.args.device),
                            'attention_mask': data[1].to(self.args.device),
                            'token_type_ids': data[2].to(self.args.device)
                            }
                q_inputs = {'input_ids': data[3].to(self.args.device),
