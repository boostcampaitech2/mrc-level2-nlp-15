...
def train(args):
    ...
    q_seqs = tokenizer(
        valid_dataset["question"], padding="max_length", truncation=True, return_tensors="pt"
    )
    p_seqs = tokenizer(
        valid_dataset["context"], padding="max_length", truncation=True, return_tensors="pt"
    )
    # print(q_seqs[0])
    ...
    optimizer_grouped_parameters = [
        {
            "params": [
                p for n, p in p_encoder.named_parameters() if not any(nd in n for nd in no_decay)
            ],
            "weight_decay": args.weight_decay,
        },
        {
            "params": [
                p for n, p in p_encoder.named_parameters() if any(nd in n for nd in no_decay)
            ],
            "weight_decay": 0.0,
        },
        {
            "params": [
                p for n, p in q_encoder.named_parameters() if not any(nd in n for nd in no_decay)
            ],
            "weight_decay": args.weight_decay,
        },
        {
            "params": [
                p for n, p in q_encoder.named_parameters() if any(nd in n for nd in no_decay)
            ],
            "weight_decay": 0.0,
        },
    ]
    ...
    for epoch in range(args.epochs):
        ...
        print(f"{epoch}epoch: loss:{losses/(step)} correct:{correct/(args.batch_size*(step))}")
    phase_vectors = []
