    ...
    def __getitem__(self,idx):
        ...
        while True:
            ...
            if ground_truth not in p_negs:
                ...
                break
        quer_inputs = self.tokenizer(quer, padding= 'max_length', return_tensors = "pt", truncation = True)
        pass_inputs = self.tokenizer(p_with_neg, padding= 'max_length', return_tensors = "pt", truncation = True)

        return {"quer_inputs" : quer_inputs,
...
def evaluate(p_encoder, q_encoder, data_path, tokenizer):
    ...
    valid_dataset = TensorDataset(valid_p_seqs['input_ids'], valid_p_seqs['attention_mask'],
    ...
    valid_q_seqs = tokenizer(query, padding="max_length",
                                 truncation=True,
...
def train(args, train_dataloader, p_model, q_model, model_checkpoint):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            a_inputs = {'input_ids': batch['quer_inputs']['input_ids'].squeeze(1).to(device),
                        'attention_mask': batch['quer_inputs']['attention_mask'].squeeze(1).to(device),
                        'token_type_ids': batch['quer_inputs']['token_type_ids'].squeeze(1).to(device), }
            b_inputs = {'input_ids': batch['passage_inputs']['input_ids'].squeeze(0).to(device),
                        'attention_mask': batch['passage_inputs']['attention_mask'].squeeze(0).to(device),
                        'token_type_ids': batch['passage_inputs']['token_type_ids'].squeeze(0).to(device), }
